...
int itkFEMRegistrationFilterTest(int, char *[])
{
  ...
  //-------------------------------------------------------------
  typedef itk::ImageFileWriter< testImageType > fileWriter;
  fileWriter::Pointer fileWrite = fileWriter::New();
  fileWrite->SetFileName("Fixed.hdr");
  fileWrite->SetInput(fixed);
  fileWrite->Update();

  fileWrite->SetFileName("Moving.hdr");
  fileWrite->SetInput(moving);
  fileWrite->Modified();
  fileWrite->Update();

  /* fileWrite->SetFileName("Init.hdr");
   fileWrite->SetInput(initField);
   fileWrite->Update();*/
  //-------------------------------------------------------------
...
//  std::cout << " input num iters, e, p: ";  std::cin >> maxiters >> e >> p;
    ...
    m->SetGlobalNumber(0);                               // Global number of the
                                                         // material ///
    m->SetYoungsModulus( registrator->GetElasticity() ); // Young modulus --
    ...
    delete e1;
    delete m;
    fileWrite->SetFileName("Register.hdr");
    fileWrite->SetInput( registrator->GetWarpedImage() );
    fileWrite->Modified();
    fileWrite->Update();
    }
