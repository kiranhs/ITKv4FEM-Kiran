...
int itkImageToRectilinearFEMObjectFilter3DTest(int argc, char *argv[])
{
  ...
  else
  {
	...
	for (unsigned int i =0; i<numberOfNodesToTest; i++)
	{
	  ...
	  if ( (fabs(femObject->GetNode(nodeNumber)->GetCoordinates()[0] - loc[0]) > tolerance) ||
	       (fabs(femObject->GetNode(nodeNumber)->GetCoordinates()[1] - loc[1]) > tolerance) ||
	       (fabs(femObject->GetNode(nodeNumber)->GetCoordinates()[2] - loc[2]) > tolerance) )
	  {
	    std::cout << "[FAILED]" << std::endl;
