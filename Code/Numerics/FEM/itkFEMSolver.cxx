...
void Solver::GenerateGFN()
{
  ...
  for ( ElementArray::iterator e = el.begin(); e != el.end(); e++ ) // step over
                                                                    // all
                                                                    // elements
    {
    ...
    if ( LoadElement::Pointer l1 = dynamic_cast< LoadElement * >( &*l0 ) )
      ...
      // changes made - kiran
        ...
        for ( ElementArray::iterator e = el.begin(); e != el.end(); e++ ) //
                                                                          ...
                                                                          // system
          {
          ( *e )->GetLoadVector(Element::LoadPointer(l1), Fe);     // ...
                                                                   // element's
                                                                   // force
                                                                   // vector
...
void Solver::AddNextElement(Element::Pointer e)
{
  this->el.push_back( itk::fem::FEMP< itk::fem::Element >(&*e) );
}

bool Solver::InsertElement(Element::Pointer e, unsigned long index)
{
  unsigned long el_size = this->el.size();

  if ( index >= el_size )
    {
    return false;
    }
  this->el.erase(this->el.begin() + index);
  this->el.insert( this->el.begin() + index, itk::fem::FEMP< itk::fem::Element >(&*e) );
  return true;
}

void Solver::AddNextNode(Node::Pointer n)
{
  this->node.push_back( itk::fem::FEMP< itk::fem::Node >(&*n) );
}

bool Solver::InsertNode(Node::Pointer n, unsigned long index)
{
  unsigned long node_size = this->node.size();

  if ( index >= node_size )
    {
    return false;
    }
  this->node.erase(this->node.begin() + index);
  this->node.insert( this->node.begin() + index, itk::fem::FEMP< itk::fem::Node >(&*n) );
  return true;
}

void Solver::AddNextMaterial(Material::Pointer material)
{
  this->mat.push_back( itk::fem::FEMP< itk::fem::Material >(&*material) );
}

bool Solver::InsertMaterial(Material::Pointer material, unsigned long index)
{
  ...
  this->mat.erase(this->mat.begin() + index);
  this->mat.insert( this->mat.begin() + index, itk::fem::FEMP< itk::fem::Material >(&*material) );
  return true;
}

void Solver::AddNextLoad(Load::Pointer ld)
{
  this->load.push_back( itk::fem::FEMP< itk::fem::Load >(&*ld) );
}

bool Solver::InsertLoad(Load::Pointer ld, unsigned long index)
{
  ...
  this->load.erase(this->load.begin() + index);
  this->load.insert( this->load.begin() + index, itk::fem::FEMP< itk::fem::Load >(&*ld) );
  return true;
}

Element::Pointer Solver::GetElement(unsigned long index)
{
  return this->el.Find(index);
}

Node::Pointer Solver::GetNode(unsigned long index)
{
  return this->node.Find(index);
}

Material::Pointer Solver::GetMaterial(unsigned long index)
{
  return this->mat.Find(index);
}

Load::Pointer Solver::GetLoad(unsigned long index)
{
  return this->load.Find(index);
}

unsigned long Solver::GetNumberOfNodes()
{
  return this->node.size();
}

unsigned long Solver::GetNumberOfElements()
{
  return this->el.size();
}

unsigned long Solver::GetNumberOfMaterials()
{
  return this->mat.size();
}

unsigned long Solver::GetNumberOfLoads()
{
  return this->load.size();
}

void Solver::ClearNodeArray()
{
  this->node.clear();
}

void Solver::ClearElementArray()
{
  this->el.clear();
}

void Solver::ClearMaterialArray()
{
  this->mat.clear();
}

void Solver::ClearLoadArray()
{
  this->load.clear();
}

Node::ArrayType& Solver::GetNodeArray()
{
  return this->node;
}

//Node::ArrayType::iterator Solver::GetNodeArrayEnd()
//{
//	return this->node.end();
//}

Element::ArrayType& Solver::GetElementArray()
{
  return this->el;
}

//Element::ArrayType::iterator Solver::GetElementArrayEnd()
//{
//	return this->el.end();
//}

Material::ArrayType& Solver::GetMaterialArray()
{
  return this->mat;
}

//Material::ArrayType::iterator Solver::GetMaterialArrayEnd()
//{
//	return this->mat.end();
//}

Load::ArrayType& Solver::GetLoadArray()
{
  return this->load;
}

//Load::ArrayType::iterator Solver::GetLoadArrayEnd()
//{
//	return this->load.end();
//}
