...
ImageToRectilinearFEMObjectFilter<TInputImage>::GetInput(void) 
{
...
ImageToRectilinearFEMObjectFilter<TInputImage>::MakeOutput( )
{
...
template <class TInputImage>
void ImageToRectilinearFEMObjectFilter<TInputImage>::GenerateData()
{
 ...
 */  
template <class TInputImage>
void ImageToRectilinearFEMObjectFilter<TInputImage>::Generate2DRectilinearMesh( )
{
  ...
  for ( double j = 0; j <= m_NumberOfElements[1]; j++ )
    ...
    for ( double i = 0; i <= m_NumberOfElements[0]; i++ )
      ...
      n = new Node(nodePoint[0], nodePoint[1]);
 ...
 */
template <class TInputImage>
void ImageToRectilinearFEMObjectFilter<TInputImage>::Generate3DRectilinearMesh()
{
  ImageConstPointer image = this->GetInput( );
  ImageRegionType region = image->GetLargestPossibleRegion();
  ImageSizeType size  = region.GetSize();
  ...
  for ( unsigned int k = 0; k < m_NumberOfElements[2]; k++ )
    ...
    for ( unsigned int j = 0; j < m_NumberOfElements[1]; j++ )
      ...
      for ( unsigned int i = 0; i < m_NumberOfElements[0]; i++ )
        ...
        e->SetNode( 3, &*femObject->GetNode( (unsigned int)( i + ( m_NumberOfElements[0] + 1 ) * ( j + 1 + ( m_NumberOfElements[1] + 1 ) * k ) ) ) );
        e->SetNode( 4, &*femObject->GetNode( (unsigned int)( i + ( m_NumberOfElements[0] + 1 ) * ( j  + ( m_NumberOfElements[1] + 1 ) * ( k + 1 ) ) ) ) );
        e->SetNode( 5, &*femObject->GetNode( (unsigned int)( i + 1 + ( m_NumberOfElements[0] + 1 ) * ( j  + ( m_NumberOfElements[1] + 1 ) * ( k + 1 ) ) ) ) );
        e->SetNode( 6, &*femObject->GetNode( (unsigned int)( i + 1 + ( m_NumberOfElements[0] + 1 ) * ( j + 1 + ( m_NumberOfElements[1] + 1 ) * ( k + 1 ) ) ) ) );
        e->SetNode( 7, &*femObject->GetNode( (unsigned int)( i + ( m_NumberOfElements[0] + 1 ) * ( j + 1 + ( m_NumberOfElements[1] + 1 ) * ( k + 1 ) ) ) ) );
        e->SetGlobalNumber(gn);
        gn++;
        femObject->AddNextElement(&*e);
        }
 ...
 */
template<class TInputImage>
void ImageToRectilinearFEMObjectFilter<TInputImage>::PrintSelf(std::ostream& os, Indent indent) const
{
