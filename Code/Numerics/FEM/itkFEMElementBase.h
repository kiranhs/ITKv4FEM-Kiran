 ...
 */
  ...
  virtual VectorType GetStressesAtPoint(const VectorType & pt,
                                        const VectorType & e,
                                        const Solution & sol,
   ...
   */
  virtual VectorType InterpolateSolution(const VectorType & pt, const Solution & sol, unsigned int solutionIndex =
                                           0) const;
   ...
   * \param solutionIndex We allow more than one solution vector to be stored - this selects which to use in interpolation.
   */
  virtual Float InterpolateSolutionN(const VectorType & pt,
                                     const Solution & sol,
                                     unsigned int f,
  ...
  DegreeOfFreedomIDType GetDegreeOfFreedom(unsigned int local_dof) const
  {
    ...
    return this->GetNode( local_dof / this->GetNumberOfDegreesOfFreedomPerNode() )->GetDegreeOfFreedom(
              local_dof % this->GetNumberOfDegreesOfFreedomPerNode() );
  }
   ...
   */
  virtual void GetIntegrationPointAndWeight(unsigned int i, VectorType & pt, Float & w, unsigned int order =
                                              0) const = 0;
 ...
 */
   ...
   */
  virtual void ShapeFunctionGlobalDerivatives(const VectorType & pt,
                                              MatrixType & shapeDgl,
