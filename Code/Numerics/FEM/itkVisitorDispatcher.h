...
template< class TVisitedClass,
          class TVisitorBase,
          class TVisitFunctionPointerType =
            ITK_TYPENAME VisitorDispatcherTemplateHelper< TVisitedClass, TVisitorBase >::FunctionPointerType >
class VisitorDispatcher
{
public:
  ...
  inline static bool RegisterVisitor(TVisitorClass *, VisitFunctionPointerType visitor_function)
  {
    ...
    if ( status )
      ...
      std::cout << "Warning: Visitor " << typeid( VisitorClass ).name() << " that operates on objects of "
                << typeid( VisitedClass ).name() << " was already registered! Ignoring the re-registration.\n";
      }
    return status;
  }
...
::Visit(VisitorBasePointer l)
{
  ...
  if ( i == Instance().visitors.end() )
    ...
    std::cout << "Error: Visitor " << typeid( *l ).name() << " that operates on objects of "
              << typeid( VisitedClass ).name() << " not found!\n";
